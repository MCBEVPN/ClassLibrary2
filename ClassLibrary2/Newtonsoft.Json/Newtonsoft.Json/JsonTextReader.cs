...
namespace Newtonsoft.Json {
	public class JsonTextReader : JsonReader, IJsonLineInfo
	{
		...
		public IArrayPool<char> ArrayPool
		{
			...
			set
			{
				_arrayPool = value ?? throw new ArgumentNullException("value");
			}
		}
		...
		private void ParseReadString(char quote, ReadType readType)
		{
			...
			switch (readType)
			{
				case ReadType.ReadAsBytes:
					{
						byte[] value2 = ((_stringReference.Length == 0) ? CollectionUtils.ArrayEmpty<byte>() : ((_stringReference.Length != 36 || !ConvertUtils.TryConvertGuid(_stringReference.ToString(), out Guid g)) ? Convert.FromBase64CharArray(_stringReference.Chars, _stringReference.StartIndex, _stringReference.Length) : g.ToByteArray()));
			...
			if (_dateParseHandling != 0)
			{
				...
				else if (DateTimeUtils.TryParseDateTimeOffset(_stringReference, base.DateFormatString, base.Culture, out DateTimeOffset dt2))
				{
		...
		private void ParseReadNumber(ReadType readType, char firstChar, int initialPosition)
		{
			...
			switch (readType)
			{
				case ReadType.ReadAsString:
					{
						...
						else if (!double.TryParse(text5, NumberStyles.Float, CultureInfo.InvariantCulture, out _))
						{
				...
				case ReadType.ReadAsInt32:
					...
					{
						switch (ConvertUtils.Int32TryParse(_stringReference.Chars, _stringReference.StartIndex, _stringReference.Length, out int value5))
						{
				...
				case ReadType.ReadAsInt64:
					{
						...
						switch (ConvertUtils.Int64TryParse(_stringReference.Chars, _stringReference.StartIndex, _stringReference.Length, out long value2))
						{
							case ParseResult.Success:
								value = value2;
								newToken = JsonToken.Integer;
							...
							default:
								...
								{
									ParseResult parseResult = ConvertUtils.DecimalTryParse(_stringReference.Chars, _stringReference.StartIndex, _stringReference.Length, out decimal value3);
